...
- **Écarter les mots que le modèle n'accentue pas nettement n'enlève aucune fausse marque** (`bench/hear.py --couples`). Sur les mêmes 100 mots, ne garder que ceux dont la syllabe élue devance la suivante d'au moins 0,50 en garde 100 et laisse les 6 ; il faut monter à 0,80, où l'on jette 10 mots, pour en perdre 3. Et les six fausses marques tombent sur des mots que le modèle accentue au maximum — `obey` deux fois à 1,00, `increases` à 0,99, `military` à 0,80. La population que le filtre visait n'existe pas, la sonde saturant. La conséquence, qui ne se lit pas dans ces chiffres : la fausse marque vient de ce que la sonde lit du **côté apprenant**, donc un seuil posé sur le modèle ne peut rien contre elle par construction. Le « seuil d'accent net » que `design/activity-model.md` réclame reste donc à écrire, mais pas ici — ce n'est pas un filtre de fausses marques.
