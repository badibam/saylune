...
**Et la brique ne se laisse pas encore écrire.** Nommer la syllabe forte marche à 84,1 % dans l'absolu ; la brique 7, elle, compare deux lectures, et là le compte est autre. Sur le corpus L2, dont chaque mot porte une note d'accent, deux lectures d'un mot correctement accentué désignent des syllabes différentes dans **28 à 36 % des cas** (`bench/accent.py`). Deux causes se cumulent, et aucune n'est réparée :

- **le montage aligné prive la réduction de tout pouvoir.** L'apprenant y est forcé sur la suite de sons du modèle : il a des plages, pas de symboles. Le test de réduction lit donc les mêmes étiquettes des deux côtés et rend forcément le même verdict — le repère à 97,5 % ne peut détecter aucune faute. Il ne reste que la durée et l'intensité.
- **le montage à deux décodages libère la réduction mais fait tourner la jointure sur la voix de l'apprenant**, où elle n'a jamais été mesurée : les 95 % de la brique 4 sont lus sur des rendus de synthèse. Ça se voit à l'œil — `like` reçoit `ɝ n l aɪ k h`, dont le début appartient au mot d'avant et la fin au suivant.

Les pistes ouvertes, et ce qui les bloque, sont dans `../TODO.md`.
