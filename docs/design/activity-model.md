...
**Un marquage au niveau du mot, binaire : l'appui est tombé sur la bonne syllabe ou ailleurs.** Le dénominateur n'est pas « les mots » : un monosyllabe n'a pas de choix d'accent, et un mot outil n'a pas d'appui net même chez le modèle. Ce sont donc **les mots de plus d'une syllabe que le modèle accentue nettement**, ce qui demande le seuil « le modèle a-t-il un accent net » que `../analysis.md` réclame déjà. La feuille n'existe pas sans ce seuil, qui définit ses éléments. Ce seuil existe et une oreille l'a validé : la marge de la sonde sur le modèle sépare les mots dont l'appui s'entend de ceux dont il ne s'entend pas (`../analysis.md`, brique 7). Ce qu'il ne fera pas, mesure faite, c'est baisser le taux de fausse marque — il n'en retire qu'une sur deux, et les autres tombent sur des mots que le modèle accentue au maximum.
...
**Les deux se marquent d'un seul dessin : une file de points médians entre les mots** — `I went · · · to the shop`. **Cinq crans, à 0,2 / 0,5 / 1 / 2 / 5 secondes**, un point par cran. Le premier cran est le seuil lui-même, donc toute pause comptée porte au moins un point. Le plus long silence est la plus longue file : il se voit parce qu'il est le plus long, on n'a pas à le désigner.

**L'échelle est en crans et non en cadence, et le dernier sature.** Un point par 200 ms ferait quarante points sur un blocage de huit secondes, mais surtout ce serait la seule mesure du projet à ne pas être en crans — la rampe des sons sature à 30 points, le débit a deux crans par côté, la pertinence en a quatre. On perd le rapport entre deux files, une pause quatre fois plus longue n'en faisant pas une quatre fois plus large, et on perd de distinguer deux blocages au-delà de cinq secondes. C'est le renoncement de la rampe, au même endroit et pour la même raison : l'image cesse de changer là où la différence cesse de vouloir dire quelque chose. Les crans servent deux feuilles qui ne regardent pas la même plage — le bas résout les petites pauses que compte la continuité, le haut les blocages, dont la série va de 1 à ~8 secondes.
...
- **Le seuil de silence à 200 ms**, partagé par la continuité et par le débit, à placer au-dessus de la plus longue fermeture d'occlusive. Et le **seuil « le modèle accentue-t-il nettement ce mot »**, qui définit les éléments de l'accent lexical : cette feuille n'existe pas sans lui. La marge de la sonde sur le modèle en tient lieu, validée à l'oreille ; ce qu'elle ne fait pas est baisser le taux de fausse marque.
