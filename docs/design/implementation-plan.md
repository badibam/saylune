...
**Ce que ça fait.** Ce que le TODO disait impossible à écrire avant le déroulé d'un module, et qui l'est maintenant. Le tour de l'IA **brouillé par défaut**, qui est la position de défaut du levier `ai-turn.display` et non une préférence, par carrés de 2 pixels posés au hasard mais fixes. Sous le tour, la rangée de commandes au padding égal, **six entrées** : le petit bouton, un **micro dans un cadre** sur trois colonnes, qui ne paraît que sur le passage ouvert — la porte du moment est dite par la ligne d'état et non par lui (`pixel-ui.md`) ; l'écoute ; le côté, une seule étiquette qui bascule ; la vitesse ; les **notes du passage**, qui ouvrent le bilan ; et la **loupe**, qui ouvre `ui/AnalysisReadout.kt` — lequel cesse d'être gardé par `Trace.on` et devient une pièce de l'app. La police porte ses phonèmes depuis le 2026-09-06 ; ce qu'il lui reste à faire est d'écrire `ʤ` et `ʧ` en deux cellules, `dʒ` et `tʃ`, la donnée gardant les ligatures que le modèle émet (`pixel-ui.md`, « Ce qu'on lui a ajouté »). Sa disposition est arrêtée (`pixel-ui.md`, « L'analyse des sons ») : une cible par rang replié, les deux écoutes dans le bloc ouvert, qui s'ouvre dans la liste et jamais en pop-up. Celle de l'écran de bilan l'est aussi, sous « Ce qui se passe entre deux tours ». Le gros bouton sur toute la largeur et trois lignes. **Trois notifications et non deux** (2026-09-06) : celle d'une règle qui change quelque chose, mécanique obligatoire et narrative facultative, qui est un **pop-up** éphémère ; celle qui dit de reprendre après une faute de son, qui donne à **écouter** le modèle et ne nomme rien ; et celle de la **porte des mots**, qui **nomme la ou les aptitudes en cause**. Les deux dernières sont des états et vivent dans la ligne d'état du haut tant qu'elles sont vraies.
