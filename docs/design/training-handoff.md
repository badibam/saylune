...
**Le notebook d'entraînement se copie, il ne se crée pas.** `speakup-training-reboot` (celui de la V1b) porte déjà les bons réglages — privé, T4 ×2, internet activé, persistance *Files only*, image Docker **épinglée** — et ses deux entrées branchées. Un *Copy & Edit* préserve tout ça **sauf la persistance, qui retombe sur « aucune » à chaque copie et se repose à la main** — mesuré, pas déduit. Une création de zéro, elle, rejouerait chaque piège. Renommer la copie, puis vérifier dans le panneau *Input* que le Dataset du code pointe la **dernière version** : une copie peut rester accrochée à celle qu'utilisait l'original, et le run tournerait alors sur un `train.py` périmé sans le dire.
...
- **Rapatrier les poids se fait par `train/fetch.py`, et c'est l'utilisateur qui le lance.** Le CLI Kaggle a trois voies et elles échouent toutes sur un checkpoint de 1,2 Go — dont deux en silence, en écrivant un `model.safetensors` de 0 octet et en sortant avec le code 0. Aucune taille rendue par le CLI ne vaut contrôle. Détail des voies mortes et de celle qui marche dans `gpu-runbook.md`.
- **Une perte négative est attendue dès que `--prior-weight` n'est pas nul**, et ce n'est pas une divergence. Les scores pénalisés partent à la perte **non normalisés** — c'est la correction qui rend le prior mordant — donc `-w·log(prior)` les relève au-dessus des log-probabilités et le CTC n'a plus de plancher à zéro. Deux bornes existent déjà : `clamp_min(1e-4)` plafonne le boost à ~9,2 nats par trame, et la mise à jour du prior est sous `no_grad`, donc aucun gradient ne passe par lui et le modèle ne peut pas se le fabriquer. Ce qu'il faut surveiller est une dérive sans borne, jamais le signe. **Corollaire : les pertes de deux runs de poids différents ne se comparent pas** — à `w=0` c'est une vraie log-vraisemblance, ailleurs elle porte un décalage.
...
1. ~~Rejouer le sortant `timit-ipa` dans la même passe.~~ **Fait** : les poids étaient dans le cache local, la lecture rend un pire témoin de 0,002 sur les deux voix, dans la fourchette 0,002–0,004 qui n'était jusque-là que citée. Toute nouvelle génération se relit contre cette lecture-là.
...
3. **Lire dans cet ordre : `boundaries.py`, puis `missing_mass.py`, puis `faults.py`.** L'ordre suit la question posée, pas l'habitude.
   - `bench/boundaries.py` d'abord, parce que la **durée couverte** est le seul chiffre que rien n'a jamais fait bouger — 26,7 à 26,9 % sur le sortant comme sur trois checkpoints d'affinage complet. C'est le procès le plus direct du prior : il mesure l'étendue, que la peakiness détruit et que l'app consomme, là où `missing_mass.py` ne dit que si la masse existe.
