...
- **Renormaliser les scores pénalisés annule le prior** : un `log_softmax` après la soustraction de `w·log prior` laisse le biais de la tête absorber la pénalité exactement, à coût nul — la perte redevient un CTC nu et la lecture brute hérite du décalage (classes rares écrasées, monotone en `w`). Prouvé sur les checkpoints de la première génération (corrélation biais ↔ pénalité jusqu'à +0,89). Les scores vont à la perte **non normalisés**, comme dans le papier ; corrigé dans `train.py`.
