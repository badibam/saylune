...
**Les deux se lisent dans la notification, après l'audio.** *« Il te reste une vie »* constate, donc c'est un reçu. Une phrase qui plante le décor du tour qui suit — *« tu te fais bousculer par un passant »* — n'en est pas une : c'est de la matière de fiction, donc un **tour scripté**, qui va dans le fil (plus bas, avec le message qui provoque un tour).
...
**Un tour scripté est un tour écrit par l'auteur, dit tel quel et sans appel au modèle.** Il a la forme d'un tour du modèle — une suite d'énoncés, chacun avec son type et son locuteur —, donc une didascalie n'est qu'une de ses lignes et rien ne le borne à une seule : c'est le plafond du tour qui borne. Il tombe aux mêmes endroits qu'un tour provoqué, et un tour scripté posé à un autre moment attend le suivant de ces endroits. Il entre dans le fil comme les autres, donc le modèle le lit dans son historique à l'appel suivant. Chaque locuteur doit être de la scène — la distribution, le narrateur, ou la voix unique d'une scène sans distribution —, ce qui se vérifie au chargement de la définition. Posé dans le même moment qu'un message qui provoque un tour, il est dit d'abord et le tour du modèle suit : deux prises du haut-parleur, avec l'appel entre elles.
...
**L'armement suit la fin d'une réponse du personnage, et rien d'autre — surtout pas l'apparition de l'écran.** L'armement tient lieu de l'appui qui ouvre un tour, donc ce qu'il suit est la conversation qui avance : le personnage a fini, c'est à toi. Un écran qui se compose n'est pas un moment de la conversation, et s'armer là-dessus ouvrait le micro par-dessus quelqu'un qui relit son fil, les horloges tournant alors sur la pièce — cinq secondes de calme envoient le tour à la troisième position, la durée entière du tour à la deuxième. Ce qui partait était ce que la pièce donnait : jeté en silence si la reconnaissance n'entendait rien, **un vrai tour avec une vraie réponse et une vraie note** si elle attrapait une toux. C'est la forme exacte du ratage contre lequel le découpage a été abandonné — une phrase que personne n'a dite devenue le texte de référence du tour.

**Il se retient donc au tour d'IA sur lequel il s'est armé**, et pas seulement au fait qu'il y en ait un : rien dans le fil ne dit si le micro a été ouvert pour la réponse qui le termine, donc sans cette trace un aller-retour vers le bilan d'un passage réarme sur la même réponse.

**Rouvrir un fil n'est pas un tour de parole, et le personnage le reprend.** L'armement n'ayant rien à suivre à l'entrée — le fil se termine sur l'apprenant, ou sur rien —, ce qui manquait était un moment, pas un déclencheur : le personnage dit une ligne qui reprend là où on s'était arrêté, et l'armement la suit comme n'importe quelle autre réponse. **Ça n'arrive qu'à l'entrée dans la conversation**, jamais au chargement de la séance qui se fait au lancement : là, c'est l'écran-titre qui est affiché, et une voix qui sortirait d'une conversation que personne n'a ouverte est précisément ce que l'app ne fait pas. Le personnage sait qu'il reprend et qu'aucune consigne ne le fait parler — sans quoi il inventerait celle qu'il n'a pas reçue — et il lui est dit de ne pas relever l'absence, dont la durée n'est ni dans le fil ni déductible.
...
**Entre la fin de la voix et le micro, un ordre fixe** (tranché le 2026-09-11). L'analyse du son se termine — elle tourne pendant la voix, dès que le jugement existe —, puis **le passage se ferme au début du décompte, pas à sa fin** : le décompte dit *tu parles dans trois secondes*, donc il n'y a plus rien à reprendre sur la phrase d'avant, et qui veut pouvoir redire choisit la capture à la main ou une porte du son qui attend. Ce que la clôture déclenche — un patch, un tour scripté ou provoqué — est dit et montré à ce moment-là. Puis la notification, sur un décompte visible, qui la retire au bout d'un plancher plus un débit, ou plus tôt au doigt ; puis la préparation, comptée dans la ligne d'état ; puis le micro. **Lire n'est pas réfléchir**, donc la notification ne mange pas la préparation : celle-ci ne part qu'une fois la notification retirée.
