...
**La conversation est une activité**, dans la même table et avec le même schéma, et tous les champs ont un sens pour elle : ses réglages sont ceux de la séance, son statut dit si elle est encore ouverte, son résultat s'agrège comme un autre. Ce qui la distingue tient à son comportement : son fil est la suite de ses énoncés, et elle se reprend en l'état. **Rien n'a à être terminé pour en commencer une autre** ; une activité non terminée est simplement une activité à reprendre, ce qui est l'état ordinaire de toutes sauf celle qu'on a.
...
**Rien dans l'app n'est nommé par le modèle** (2026-09-07). Une activité s'appelle comme la **définition** dont elle est ouverte, qui déclare son nom en deux longueurs — celui que porte une tuile, et celui que la ligne d'état peut tenir. Une séance n'a donc pas de champ de nom à elle : ce qui l'identifie est son origine, qui nomme la définition et sa version.
...
**Une tentative remplace à l'affichage, jamais en base.** Le fil ne montre que la dernière, et aucun écran ne montre les précédentes ; elles restent toutes sous le passage, une tentative effacée étant une mesure perdue (`design/activity-model.md`).

**Et les tentatives s'arrêtent à la clôture du passage**, les deux sortes. Le passage ne se ferme qu'au geste de l'apprenant, donc retravailler sa phrase est toujours possible — il suffit de ne pas passer à la suite. Une fois fermé, il se réécoute et se relit pour toujours : une redite ajoutée après coup déplacerait sa note sous des règles qui l'ont déjà lue, et une reformulation referait cinq tours trop tard une réponse sur laquelle la conversation a déjà bâti. Le travail hors du fil est un autre mode, qui n'écrit rien (`../TODO.md`).

**Une reformulation refait la réponse de l'IA ; une redite ne la touche pas.** Reformuler donne au modèle une phrase neuve, donc il y répond neuf, et la réponse précédente disparaît du fil — que la conversation ait attendu sur la phrase ou qu'elle ait poursuivi. Redire ne change pas les mots, donc il n'y a rien de neuf à répondre : c'est un exercice, qui ouvre l'analyse du son sans rien rejouer du fil. Ce que l'app ne fait pas, c'est jouer une réponse puis la contredire sur la même phrase : dans une tentative, elle reçoit la continuation et l'écho ensemble et n'en joue qu'un.
