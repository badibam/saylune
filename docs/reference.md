...
**La synthèse porte une contrainte structurelle : l'ancrage horodaté.** C'est de lui seul que vient l'ancrage aux lettres, donc l'exigence 2 ci-dessus en dépend entièrement. ElevenLabs le rend **caractère par caractère en REST nu** ; Azure rend l'équivalent — frontières de mots, visèmes — uniquement par un SDK propriétaire, ce qui heurte de front la publication F-Droid. **Un seul fournisseur convient donc aujourd'hui**, et c'est l'attache la plus étroite de tout le montage : l'analyse est à nous, mais l'endroit où poser ses marques ne l'est pas. **La direction pour la dissoudre est tranchée** : l'ancrage se calculera sur l'appareil, par un aligneur de caractères sur le même runtime — lettres et sons joints par le temps, sans table graphème-phonème — et l'alignement fournisseur cessera d'être consommé dès que cet aligneur se qualifie, rendant tout TTS candidat (cf. `../TODO.md`, chantier 2).
