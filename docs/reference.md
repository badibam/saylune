...
- `scene-authoring.md` — **écrire une scène**, pour les quatre portes : ce que toute scène doit tenir pour valoir la peine, ce que chaque porte permet, et pourquoi le schéma s'y lit dans le code au lieu de s'y recopier. Suivi tel quel par `./run scene`.
...
- `design/` (`design/grammar-test-set.md`, `design/local-chain.md`, `design/local-recognition.md`, `design/small-acoustic-model.md`, `design/premium-chain.md`, `design/remote-analysis.md`, `design/voices.md`, `design/distribution.md`, `design/harvest.md`, `design/scene-state.md`, `design/scene-vectors.md`) — le transitoire, qui s'élague quand il est consommé : les énoncés des deux bancs du chantier 2, qui n'ont jamais tourné ; le montage où la voix des personnages devient locale, avec les critères de ses mesures posés avant les chiffres ; la reconnaissance sur l'appareil, où le verbatim tient l'analyse du son et où rien de libre ne le rend, donc s'entraîne ; l'essai d'un modèle acoustique trois fois plus petit, ouvert parce que la passe est trop longue sur un tour de vraie longueur et que les deux petits modèles déjà mesurés ont échoué pour des raisons d'entraînement et non de taille ; la coupe de l'appel au modèle en un qui parle et un qui juge, qui ferme deux dettes du prompt et sur laquelle s'adosse un mode payant passant par un seul fournisseur ; la séparation de la voix qui sert d'étalon et des voix de ceux qui parlent, ces dernières tirées au hasard hors du mode payant et nommées dedans dans un catalogue tenu par l'app ; les deux versions distribuées de l'app, le compte anonyme qui tient les crédits, les deux rails de paiement et la livraison d'une histoire payante ; ce qu'une passe d'analyse coûte sur une machine qui n'est pas le téléphone, quatre fois moins de temps sur un serveur à quatre cœurs, avec la recette de la machine qui l'a rendu et la latence toujours pas mesurée ; trois leçons d'établi qui attendent d'être poussées vers la sagesse ; et un seul modèle pour ce qui fait bouger une scène — des cases, qui les écrit, des événements qui les lisent, des textes vers quatre destinataires — qui remplace la vingtaine de notions des règles et des questions ; et les sortes de scènes où ce modèle se met à l'épreuve, tirées de dix dimensions croisées deux à deux.

## Le geste

L'utilisateur parle anglais à voix haute. L'IA lui répond en voix, dans l'accent choisi. Rien ne l'interrompt.

Quand il se trompe, l'IA reprend **dans sa réponse** au lieu de s'arrêter — à qui vient de dire *I have 25 years*, elle répond « Ah, you're 25! And where... ». La reprise passe dans le fil ; le tour fautif porte en plus une **marque discrète**. Sans cette trace, la discrétion se retourne : on corrige, et personne ne l'apprend.
...
**L'énoncé est l'unité de matière** : un texte, un locuteur, l'audio, l'analyse. Toute parole dans l'app en est un — la réponse de l'IA comme la phrase de l'apprenant. La plupart n'en portent aucune analyse, et c'est voulu : une seule table pour tout ce qui se dit donne le fil de la conversation gratuitement, comme simple suite ordonnée, et aucun champ ne le porte. Un énoncé peut **pointer vers celui qu'il reprend**, par son identité et jamais par sa place, une place ne survivant pas à l'écriture sur disque.

**Une activité est atomique** : un seul objet, qui porte ses réglages, ce qu'il regarde, ses règles, son statut, ses horodatages et son résultat. Ce n'est pas que la séparation définition/exécution serait inutile — rejouer s'en sert — c'est qu'elle n'a pas à être modélisée comme une activité, la définition étant de la **donnée** et la partie une ligne en base (`activity.md`). Les énoncés s'accrochent à l'activité, **toujours à un seul parent**, ce qui garde chaque requête sur la matière simple.

**La conversation est une activité**, dans la même table et avec le même schéma, et tous les champs ont un sens pour elle. Ce qui la distingue tient à son comportement : son fil est la suite de ses énoncés, et elle se reprend en l'état. **Rien n'a à être terminé pour en commencer une autre** ; une activité non terminée est simplement une activité à reprendre, ce qui est l'état ordinaire de toutes sauf celle qu'on a.
...
**Une seule barre, la même partout : A ou B, ça va.** Sur un passage elle décide s'il faut redire, sur une activité si c'est réussi et si le niveau suivant s'ouvre. L'apprenant apprend la règle une fois et elle vaut partout. **Et elle ne se règle pas** : si la sévérité et la barre bougeaient toutes les deux, deux boutons feraient la même chose et plus rien ne dirait lequel a rendu une séance dure. Ce qui se règle est la sévérité ; ce qu'il faut atteindre ne bouge jamais.

**Perte assumée** : il n'y a pas de conversation sans aucune marque grammaticale. C'est un retour au texte d'origine — sans la trace, la discrétion se retourne.
...
Un curseur d'aptitude n'est pas un paramètre : c'est un **préréglage** sur des **leviers**, qui sont les vrais paramètres et ce qui se stocke. Et ce n'est qu'un préréglage parmi d'autres — *easy* et *hard* en arcade en sont, chaque mode aura les siens (`activity.md`).

**Aucun réglage ne touche une marque.** Ce qu'ils gouvernent, c'est ce qu'on en **fait** — s'il faut redire, si ça compte dans la note. Une marque dont l'absence dépendrait du réglage du jour ne transporte plus rien : rien ne la distinguerait d'une approbation.

**La note vit sur la mesure, pas sur l'aptitude** : une aptitude est un regroupement de mesures, chacune avec sa sensibilité et son poids (`measures.md`).

**Correction et pertinence sortent d'un seul marquage par groupe de mots**, et chacune est **une feuille** dont les crans sont ses étiquettes. La correction demande *est-ce que c'est de l'anglais* ; c'est un jugement absolu contre une norme **fixée par l'app et identique pour toutes les activités**, et c'est cette invariance qui le rend vérifiable au banc sur des phrases isolées. La pertinence demande *as-tu visé juste*, et c'est elle qui porte la consigne, donc tout ce qu'une activité veut exiger. C'est la seule mesure du projet qui ait un bon côté : il n'y a pas de modèle unique de la bonne phrase, donc on peut dépasser le simplement correct, là où pour le son être sur le modèle est déjà le mieux qu'on puisse faire.
...
**Ce qui va être réécrit et la prononciation ne se marquent pas côte à côte : l'analyse sonore ne tourne pas sur une phrase qu'on va reformuler.** Rien à reformuler, l'analyse est visible immédiatement ; une reformulation attendue, la marque seule et aucune analyse ; une fois la phrase reformulée et redite, l'analyse apparaît sur ce nouvel énoncé.

**Il y a une porte par sorte de réparation.** La **porte des mots** se lit au retour de l'appel et dit que le passage est **à reformuler** ; la **porte du son** se lit à la fin de l'analyse et dit qu'il est **à redire**. Toutes deux lisent la **note d'une aptitude** à la barre A–B.

**Ce qui range chaque aptitude d'un côté est câblé** : celles dont l'échec fait changer les mots — la correction, la pertinence, la compréhension — ferment la porte des mots ; celles dont l'échec laisse la phrase intacte — l'élocution, la fluidité — ferment celle du son. Une activité choisit **de quelles aptitudes elle fait refaire**, jamais de quel côté ça tombe. Coût assumé au réglage le plus lâche : le modèle dira d'une voix native une tournure qu'on a choisi de ne pas reprendre.

**Ce qui coupe l'analyse du son n'est pas une troisième porte** : elle ne tourne pas si la porte des mots s'est fermée, ou si un mot est marqué `ne se dit pas` — **et ce second cas n'est pas une décision, c'est une absence de sol.** Une phrase qui n'existe pas dans la langue **ne peut pas** être synthétisée : la faire dire au modèle donnerait à imiter une non-phrase. On ne peut pas non plus retirer l'empan pour synthétiser le reste, comme on retire une hésitation : un bafouillage est hors de la phrase, une non-phrase est dedans et demande à être remplacée.
...
L'analyse tourne donc sur une phrase **jugée assez correcte au réglage courant**, et rien de plus : son texte de référence est aussi sûr ou incertain que celui de n'importe quel autre tour. Le détail — quelle feuille ferme quoi, ce que mesure un tour sans mots retenus, le blocage — est dans `activity.md`.
...
Cette règle ne contredit pas le fait que **nommer le son produit ne sert jamais de verdict**. Les deux se complètent : l'étiquette ne dit pas si c'était *correct* — ça, c'est l'écart au modèle qui le dit —, et elle dit ce qui a été *produit*, ce que rien n'a le droit de récrire.
...
La syllabe **porte** l'accent et la hauteur, mais elle n'est la portée d'aucun des deux : l'accent est une propriété du mot, fixée par la langue indépendamment de la phrase ; la mélodie n'existe qu'à l'échelle de l'énoncé.

**Les trois se jugent par comparaison au modèle, jamais dans l'absolu.** Ce n'est pas une précaution, c'est la forme même de l'analyse : rien d'extérieur aux deux enregistrements n'est jamais consulté — ni dictionnaire de prononciation, ni lexique de dialecte, ni référentiel de justesse. La seule question posée est *en quoi cette prise s'écarte-t-elle de celle-là*, par la même machine, dont le biais est donc le même des deux côtés et s'annule.

**Les trois s'ancrent aux mêmes caractères du texte affiché** — chaque son connaît les lettres qu'il couvre, chaque syllabe sa sous-chaîne, et la mélodie se pose sur ces mêmes groupes. Ce sont trois propriétés d'une seule chaîne, pas trois analyses à superposer. C'est ce qui rend possible de les marquer **d'un seul mouvement**, et la forme retenue le fait : **la couleur ne porte jamais l'échelle**, qui est portée par la forme — les lettres teintées pour le son, un filet sous la syllabe pour l'accent, une bande au-dessus de la ligne pour la mélodie, une vaguelette pour la correction, des crochets pour la pertinence. La couleur porte le côté et la distance au neutre : l'alarme partout, et le vert des deux crans hauts du projet. Le dessin complet est dans `ui.md`.

Trois irrégularités à prévoir : une lettre peut porter deux sons, une lettre peut n'en porter aucun, et **un son peut ne porter aucune lettre**. La troisième est mesurée à 1,4 % des sons du banc, et se sépare en deux causes qui n'appellent pas le même geste — la lettre déjà prise par le son voisin (le `x` de `boxes`), et la voyelle que l'orthographe n'écrit pas du tout (le schwa de `doesn't`).
...
Et ce qu'on compare n'est jamais deux notes, mais **deux formes** : à chaque instant, la répartition de la ressemblance sur tous les sons de l'anglais. `R 0,90 / W 0,10` et `R 0,90 / ER 0,10` ont le même pic et ne disent pas la même chose.

Mesuré (`analysis.md`) : sur le jeu d'essai étiqueté, les prises témoins restent à 0,003 d'écart du modèle et les fautes franches montent au-dessus de 0,93. L'étalon annule le bruit propre de la machine, ce qui supprime tout besoin de calibrer son par son.
...
**Et la symétrie d'application ne suffit pas à rendre un traitement neutre.** Un micro dans une pièce d'un côté, une synthèse propre de l'autre : un débruiteur appliqué aux deux ne ferait presque rien sur le modèle et beaucoup sur la prise — symétrie du geste, asymétrie de l'effet. C'est un argument pour en faire le moins possible, pas pour en faire des deux côtés.

Ce qui suit n'est pas du traitement mais le contrat d'entrée : 16 kHz, mono, PCM 16 bits. Ce qui est refusé nommément : gain automatique, suppression de bruit, annulation d'écho. La capture demande donc au système sa source **sans traitement** quand l'appareil déclare en avoir une, la source de reconnaissance sinon — et elle **dit laquelle elle a obtenue**.

**Le seuil est une exigence, pas un quota.** Une barre unique sur l'écart, la même pour chaque son, partout et tout le temps. Jamais « marquer les n pires », jamais un budget par mot ou par tour : **toutes les fautes se marquent, aucune ne s'élit**. Un quota serait une élection, et il ferait dépendre une marque de ce qu'il y a d'autre dans la phrase.
...
**Et cette barre ne se règle pas.** La rampe est la même pour tout le monde et tout le temps, et la bande de bruit sous laquelle rien n'est teinté est une propriété mesurée de la machine, pas un réglage. Le réglage d'élocution gouverne ce qu'on **fait** d'une marque, jamais son apparition. Les trois échelles ont chacune la sienne, leurs unités n'étant pas les mêmes ; celle de la mélodie est **géométrique** — le contour de l'apprenant est peint sous celui du modèle, à la même épaisseur, donc un écart plus petit que le trait ne laisse rien dépasser (`ui.md`).

**Se taire n'est jamais une issue.** Quand un repère est trop bruyant, la réponse est de le réparer ou d'en trouver un autre — jamais de restreindre la marque aux cas faciles ni de retirer une échelle. Une échelle qui ne parle que là où elle est sûre laisse passer les fautes qu'elle est là pour attraper, et l'apprenant n'a aucun moyen de savoir que le silence n'est pas une approbation. Décision de projet, à ne pas rouvrir. Sa portée, notée pour que la question ne se repose pas : il a été écrit contre l'idée de restreindre une échelle **en service** à ses cas faciles ; il n'ordonne pas de retenir l'app derrière une échelle qui n'est pas encore écrite. L'accent lexical est resté de la recherche tant qu'il n'était pas fiable ; sa fiabilité est maintenant mesurée et la brique est dans l'app entière, sans restriction.

**Une marque est une proposition d'aller voir, pas un verdict.** L'écart au modèle est validé là où l'apprenant a **entendu** ce modèle avant de parler : deux fautes d'accent sur deux vues, aucune fausse alerte sur les calques (`qualification.md`, bloc F). Sur un tour de conversation il n'a rien entendu, et l'écart s'affiche quand même.

**La règle vaut aux trois échelles, sans exception : le modèle est la source de vérité, et s'en écarter se marque.** On vise l'identique, au seuil près. Ça n'interdit rien à l'apprenant : une phrase spontanée admet plusieurs mélodies justes, et qui veut mettre l'emphase sur un mot la met — sa courbe divergera du modèle neutre, il la verra diverger, et il en fait ce qu'il veut. L'app montre l'écart ; elle ne prétend pas savoir ce que l'apprenant voulait dire.
...
**Un modèle n'est un étalon que s'il s'étalonne.** Une voix de synthèse que l'analyse lit mal accuserait l'apprenant d'une faute commise par la machine — le principe se paie ici précisément parce que le modèle est cru aveuglément. Toute voix promue modèle passe donc un test, et une voix qui échoue est écartée quelle que soit sa beauté.
...
La position se lit des deux côtés gratuitement : la grille dit où le modèle prononce chaque son, l'alignement dit où l'apprenant le prononce. Le modèle à jouer et **l'enregistrement de l'apprenant au même endroit** tombent du même calcul.

Un extrait de son ne se coupe jamais à ses bornes exactes : ce qui identifie une occlusive vit dans la transition vers le son suivant, donc on joue une marge de part et d'autre — large autour d'un son, étroite autour d'un mot, où ce qui déborde est le mot voisin. Ralentir garde la hauteur : rééchantillonner descendrait les formants et changerait la voyelle entendue.
...
Redire produit **un énoncé de plus dans la même conversation**, pointant vers celui qu'il reprend. Rien ne s'interrompt, rien ne part travailler ailleurs, et rien n'a à se refermer. Ce qui s'y dit est noté contre un texte de référence **connu d'avance** — celui de l'énoncé repris — ce que la conversation libre ne peut pas offrir.

**Le nombre de marques ne multiplie pas le travail.** L'unité de réparation est la phrase, le mot au plus ; jamais le son. Trois sons marqués dans un mot font un seul mot à redire. Les marques disent **où porter l'attention** en redisant, et c'est ce qui rend tenable de tout marquer sans rien élire.

Le prix se paie sur l'**affichage** et non sur la charge : mesuré sur de la parole d'apprenant réelle, la majorité des mots portent quelque chose. Une marque binaire ne transporte alors plus rien — tout est colorié, donc rien n'est signalé. Le marquage **gradue** donc, et c'est en service : une rampe d'ambre à rouge à mesure que l'écart grandit, à teinte claire fixe pour qu'elle se lise comme une seule famille. Sous la bande de bruit la lettre garde l'encre neutre — une teinte y serait un mensonge de précision.

**Une reformulation refait la réponse de l'IA ; une redite ne la touche pas.** Reformuler donne au modèle une phrase neuve, donc il y répond neuf. Redire ne change pas les mots, donc il n'y a rien de neuf à répondre : c'est un exercice, qui ouvre l'analyse du son sans rien rejouer du fil. Le détail — les tentatives, les deux boutons, la clôture — est dans `activity.md`.

## Les deux tuyaux

Deux circuits distincts partagent une seule ressource : le fichier audio du tour de parole.

**Tuyau A — la conversation.** Micro → tampon local (PCM 16 kHz mono, **conservé**) → fin de parole → envoi au fournisseur → lecture de la réponse audio. Le tampon local est la condition d'existence du tuyau B : aucun service distant ne rend l'audio envoyé. **Le tour part d'un seul tenant, tel que le micro l'a entendu.**

**On envoyait la parole seule, et on ne le fait plus** (tranché le 2026-09-10). Le tour était découpé sur une barre de volume absolue, la parole collée bout à bout pour le réseau et le reste rendu en silence numérique pour le disque. Une barre absolue ne tient pas d'un appareil à l'autre : l'app demande la source de micro sans gain automatique, donc la même voix arrive à des niveaux qui diffèrent de plus que la marge que la barre laissait. Sur un appareil plus faible que la barre, la parole passait pour du silence — ce qui était gardé revenait haché, et ce qui partait était les quelques syllabes survivantes, que la reconnaissance transcrivait fidèlement : **une phrase que personne n'a dite devenait le texte de référence du tour**, sans que rien ne le signale. En face, le découpage n'achetait que des octets d'envoi et des secondes facturées, la reconnaissance se payant à la durée. Le silence part donc au réseau, et il est payé.

**Tuyau B — analyser.** Le tour est examiné, **sur l'appareil**, pour savoir s'il y a un problème et où. Se fait sur l'enregistrement existant, sans jamais rien redemander. L'audio d'un tour ne quitte donc l'appareil **qu'une fois**, pour la conversation.
...
**La portée de l'invariant.** Il a été écrit contre l'app qui coupe dans le dos de quelqu'un en conversation ordinaire ; il vaut donc là où rien n'a été déclaré. Une pression annoncée est un autre problème : la troisième position de capture coupe, sous une règle connue d'avance et choisie. Elle ne contredit pas l'invariant, elle est hors de sa portée.

La capture est un **levier de fluidité à trois positions**, décrit dans `activity.md` ; trois choses y sont décidées quelle que soit sa position : le micro **s'arme sur la fin d'une réponse de l'IA, et sur rien d'autre** — jamais avant elle, et jamais parce qu'un écran s'affiche (`activity.md`) ; un symbole est visible dès que ça enregistre ; et **chaque tour porte sa position de capture**, sans quoi rien ne dit si ses silences sont significatifs, et rien ne s'agrège entre positions.
...
La conversation dépend de services distants, et chacun peut manquer. L'analyse, elle, ne dépend que de l'appareil. La règle se décline par brique, pas globalement :
...
- **Un juge qui lâche après que la voix est sortie ne se répare pas en renvoyant la prise** — le personnage a déjà répondu, il répondrait deux fois. Ce qui manque est **un appel**, et tout ce qu'il prend est encore en main : c'est lui qu'on redemande, sous le même bouton, parce que l'apprenant n'a qu'une question — ça n'a pas marché, refais. **La conversation est retenue en attendant** : un tour à qui on a répondu et que personne n'a lu glisserait dans le passé, et les marques arriveraient devant quelqu'un qui parle d'autre chose. Rien ne s'arme et rien ne s'ouvre tant que ça tient.
- **Et cette réparation ne vit que le temps de la séance.** Quitter la conversation et y revenir **tue le tour non lu et la réponse qui lui a été faite** : le fil reprend un cran avant, là où il était encore entier. Ce que le second appel demande vit en mémoire à côté de l'état effectif, qui ne survit pas non plus à une reprise ; et redemander un jugement sur un tour qu'on a quitté est exactement l'analyse différée que ce doc refuse. C'est **le seul endroit de l'app qui efface un énoncé**, et la règle inverse tient partout ailleurs pour la raison qui manque ici : une réponse supplantée est ce qui a vraiment été dit et un banc la relit, là où un tour que personne n'a lu ne porte ni marque, ni mesure, ni note.
- **L'analyse ne dépend d'aucun réseau**, ce qui retire d'un coup le quota, la clé expirée et le fournisseur en panne. Ce qui reste est d'un autre genre et se règle **une fois pour la conversation, pas à chaque tour** : les poids ne sont pas téléchargés, ou l'appareil ne parvient pas à les charger. Les marques sonores sont alors éteintes de bout en bout, et l'option éteinte **porte sa raison**. Un tour reste analysable tant que la conversation a démarré avec son modèle chargé.
- **Redire a besoin de la synthèse, pas du réseau d'analyse.** Redire sans modèle à entendre n'est pas un exercice dégradé, c'est de l'auto-évaluation à l'oreille — ce que l'architecture refuse partout ailleurs. Si la synthèse manque et que le cache ne porte pas la phrase, l'app le dit franchement.
...
L'analyse compare l'audio de l'apprenant au modèle synthétisé pour un **texte donné**. En conversation libre, ce texte n'est pas connu d'avance, et c'est la seule contrainte de la chaîne qu'on ne peut pas contourner : les mesures fines n'existent que là où le texte est écrit d'avance. La porte des mots en retire déjà la moitié du problème. Reste le tour qu'on garde tel quel.

**Décision** : le texte vient de la transcription, que le LLM peut corriger à partir du contexte. La répartition des rôles est une règle : **le STT transcrit la bouche, le LLM décide l'intention.** La normalisation appartient au LLM, qui a le contexte et une instruction — jamais au STT, qui la ferait en silence ; un STT qui répare la grammaire d'office efface le signal d'apprentissage avant tout jugement, et sa fidélité verbatim est un critère de choix (`../TODO.md`).
...
La ponctuation d'`intended` fait partie de cette tâche : le LLM ponctue selon l'intention à laquelle il répond, et le contour du modèle TTS en dépend.

**Et le texte ne porte jamais de chiffres.** Nombres, dates et montants s'écrivent en toutes lettres — `twenty five`, jamais `25`. Ce n'est pas une préférence d'affichage : l'analyse pose chaque son sur les lettres qui l'écrivent, et un chiffre n'en a aucune. Mesuré : sur `I am 25 years old`, le mot `25` sort entièrement du découpage et ses huit sons sont récupérés par `years`, dont toutes les marques tombent alors à côté. Ce seul mot faisait **24 des 53 sons sans lettre** des 95 rendus du banc ; en toutes lettres il en reste 18, soit 1,2 % au lieu de 3,4 %. La règle vaut partout où un texte de référence se fabrique.

Deux faits mesurés encadrent ce choix. Le piège se déclenche moins souvent qu'on le craignait : sur *« I sink »* comme sur *« I am walkin »*, la reconnaissance a rendu `think` et `walking` d'elle-même. Mais **une reconstruction fausse coûte** : *« Turn light at the corner »* reconstruit en `left` au lieu de `right`, le contexte admettant les deux, rend quatre sons aberrants d'un coup. Ce coût avait été écrit plus grand qu'il n'est — le doc annonçait une **rafale**, et mesuré depuis, une faute ne se propage pas (`analysis.md`, brique 11). Le cas propre du texte faux n'est pas mesuré pour autant, aucune prise du jeu n'en portant.

**Le garde-fou qui en découlait est à revoir.** Il devait écarter un tour où beaucoup de sons s'effondrent ensemble, au motif que ça signale plus probablement un mauvais texte qu'un mauvais locuteur. Cette inférence ne tient pas : un apprenant très accentué s'écarte lui aussi partout — mesuré sur une prise du jeu, où sur sept différences relevées une seule est la faute voulue et six sont des traits d'accent. Le garde-fou éteint alors l'analyse sur ceux qui en ont le plus besoin, **et sans le dire**. Ce qui le remplacera n'est pas décidé ; une piste : **marquer le mot**, l'écran affichant le texte transcrit, donc l'erreur est bornée à un mot et lisible par qui la reçoit. Resterait à contrôler l'**alignement dégénéré**, qui se lit à des durées absurdes sans rien inférer sur le texte.
...
**On stocke ce qui dépend de quelque chose qui ne se retrouvera pas** — l'audio d'un moment, le jugement d'un juge, une version de modèle. **On recalcule tout ce qui ne dépend que des lignes** : le nombre d'énoncés, leur ordre, les durées. Ni les lectures d'un tour, ni le modèle à imiter, ni le cran de la prise suivante n'ont donc de colonne.
...
- **La voix de l'apprenant** n'est **pas purgée par défaut**, et la purge reste à écrire (`activity.md`). Un enregistrement qu'aucun énoncé ne nomme, lui, est effacé : c'est du rebut, venu des deux tours qui finissent sans énoncé — celui où la reconnaissance n'a rien entendu, celui dont la chaîne a cassé sans réessai. Le balayage se fait **au démarrage et nulle part ailleurs** (`store/Recordings.kt`), seul moment où rien n'est en vol et où un fichier que la base ne nomme pas ne sera jamais nommé.
- **Le cache des synthèses** survit, sous **plafond réglable avec éviction du moins récemment demandé**, 500 Mo par défaut : ce n'est qu'un cache, régénérable au prix d'un appel, et ce qu'il vaut d'en garder dépend du téléphone. Une synthèse rendue est touchée à chaque fois qu'on la redemande, sans quoi la date dirait quand elle a été fabriquée et la phrase voulue tous les jours sortirait avant une phrase que personne ne réclame plus.
- **La trace écrite** — les énoncés, qui parle, le texte, les marques avec leurs ancres — est archivée **telle que l'écran l'affiche**, sans modélisation ajoutée : des faits bruts, dont les usages futurs se dériveront s'ils adviennent. Elle **se relit** : une conversation se reprend là où elle en était.

**Ce que ça pèse, mesuré sur le téléphone** (SM-G975F, 2026-09-01, 99,5 Mo produits par l'usage) :

| tas | où | n | total | médiane | max |
|---|---|---|---|---|---|
| tours gardés (`takes/`) | externe, build debug seul | 365 tours | 54,9 Mo | 144 Ko | 528 Ko |
| tampon du tuyau A (`turns/`) | interne | 408 wav | 29,6 Mo | 64 Ko | 408 Ko |
| cache des synthèses | interne | 137 wav | 15,0 Mo | 96 Ko | 520 Ko |

S'y ajoutent `matrices/` (184 Ko) et `probe/` (3,0 Mo), qui ne croissent pas avec l'usage. **Le plus petit tas est le seul que le doc veuille garder** : la trace écrite fait 16 Ko par tour, donc mille tours font 16 Mo, ce qui ne demande aucun plafond. Le plafond du cache est un garde-fou contre une dérive, pas un serrage — à 96 Ko la médiane, 500 Mo tiennent environ cinq mille phrases. Et **la voix de l'apprenant était gardée deux fois**, `Takes.keep` copiant le wav qui vit déjà dans `turns/` ; c'est un lien dur depuis, avec la copie derrière quand les deux dossiers ne sont pas sur le même système de fichiers.

**Ce que l'éviction casse, et ce qu'on en fait.** La base **nomme des fichiers du cache**, donc évincer rend le chemin pendouillant. Décidé : **l'analyse reste, la phrase n'est plus rejouable, point.** Pas de resynthèse — elle coûterait des jetons et du carbone pour rendre un audio qui n'est plus l'étalon de la mesure, la version Replicate n'étant pas épinglée. Pas d'analyse supprimée non plus : une mesure survit à l'audio dont elle est tirée. **Ordre d'éviction : les prises de l'apprenant d'abord, les rendus en dernier** — une prise ne coûte rien à ne pas régénérer, un rendu si, et c'est le modèle qu'on réécoute pour l'imiter.
...
Ce choix n'est pas qu'économique : il est la seule issue compatible avec la publication sur F-Droid, qui interdit toute clé embarquée dans une release.
...
- **Le catalogue se récupère à chaque ouverture de l'écran, sans cache — et c'est lui, la sonde.** Demander à un fournisseur les voix qu'il propose valide la clé du même geste : une liste qui revient est une clé qui marche, et la liste revenue est ce que *cette* clé débloque vraiment. Un catalogue mis en cache, lui, périt en silence. Le fetch ne bloque pas : seul le sélecteur qui attend dit qu'il attend.
- L'utilisateur paie sa consommation : l'app doit pouvoir dire ce qu'elle consomme — la reconnaissance, le modèle de langue et la synthèse.

Limite connue et acceptée : le BYOK reste un mur d'adoption. Le mur est tenable parce que **l'app est d'abord pour son auteur** : la publication F-Droid est une générosité et une discipline, pas une stratégie d'adoption.

**L'analyse, elle, se paie autrement** : un téléchargement unique de 359 Mo de poids acoustiques, en **opt-in explicite** au premier usage — jamais au premier lancement, jamais en silence. C'est un mur d'un autre genre, franchi une fois, et les poids sont sous licence libre (Apache-2.0), sans quoi ils seraient un `NonFreeAssets`.

## L'analyse est à l'app, les autres briques sont à des fournisseurs

**L'analyse tourne sur l'appareil, et c'est la colonne vertébrale de l'app** (`analysis.md`). **Elle ne consulte aucune norme qui juge** : ni dictionnaire de prononciation, ni lexique de dialecte, ni référentiel de justesse.

Deux fichiers extérieurs dans toute la chaîne : les poids d'un modèle acoustique libre, et une **table d'affinité graphème↔phonème** de quelques kilo-octets — laquelle ne juge rien. Elle sert à la seule brique du marquage : savoir si `s` participe à /ʃ/, donc sur quelles lettres poser une couleur déjà décidée ailleurs. La distinction porte tout le principe : une norme extérieure qui dirait ce qui est *correct* est refusée ; une table qui dit où *peindre* ne l'est pas, et sans elle la marque tombe à côté près de deux fois sur cinq.

Ce montage ne ressemble pas à l'approche habituelle, et pour une raison de situation plus que d'astuce. Qui n'a que l'audio d'un apprenant et un texte a besoin d'un dictionnaire pour se donner une norme. L'app, elle, possède **deux enregistrements du même énoncé** — elle synthétise le modèle de toute façon. Le modèle est donc la source de vérité, cru aveuglément, et rien d'extérieur aux deux enregistrements ne juge quoi que ce soit.

Mesuré : les fautes du jeu d'essai se trouvent sans qu'aucun témoin ne se déclenche, et la chaîne **tourne sur un téléphone de 2019** (`analysis.md`).
...
3. Rendre son jugement aux **trois échelles**.
...
**Dire quel son a été produit** est un enrichissement, non une exigence.

**Conversation et synthèse restent distantes, et substituables** — `NonFreeNet` reste déclarée à ce titre.

**Le montage est la chaîne STT → LLM → TTS**, tranchée, plutôt qu'une API voix-à-voix. Trois raisons, dont la dernière est mesurée : chaque maillon reste substituable ; la reconstruction du texte de référence voyage dans l'appel LLM qu'on fait de toute façon, là où le voix-à-voix exigerait un appel supplémentaire par tour ; et la latence se mesure maillon par maillon (`providers.md`).

**La synthèse ne doit rendre qu'un audio.** L'exigence 2 se calcule entièrement sur l'appareil, par l'ordre et l'orthographe. Rien n'est demandé au fournisseur qu'un wav, donc **tout moteur de synthèse est candidat**, les libres compris.

**Les fournisseurs de conversation et de synthèse restent à choisir** (`../TODO.md`). **Ce qui est tranché, c'est que le choix appartient à l'utilisateur, brique par brique.** Chaque maillon a son sélecteur, et les mélanger est l'état normal plutôt qu'un cas limite. Une même clé peut servir deux maillons — Replicate porte la reconnaissance et la synthèse — et le mur du BYOK baisse d'autant. Un fournisseur n'est offert à un maillon que si sa clé est saisie ; c'est un filtre local, sans aucun appel.

**Ce qui compte là est la seule information que l'utilisateur seul détient — une clé, une région.** L'adresse du service n'en fait pas partie : c'est un remplacement avec un défaut derrière lui, donc la laisser vide est le cas ordinaire. La distinction se porte **sur l'entrée elle-même** et jamais dans une liste tenue à côté : une liste est un second endroit à mettre à jour, et le fournisseur qu'on y oublie n'apparaît simplement jamais dans son menu, sans que rien à l'écran ne dise pourquoi — c'est arrivé, à l'ajout d'OpenAI.

Deux conséquences qui se paient dans le code. Le **modèle est un paramètre par maillon**, jamais un attribut de la clé, puisque la même clé sert deux tâches. Et **la disponibilité se demande par maillon** : la clé Azure est indispensable à qui reconnaît chez Azure et sans objet à qui reconnaît chez Replicate.

**Ce qu'un fournisseur rend en plus ne peut jamais devenir une condition.** La reconnaissance en est le cas concret : whisperx rend les bornes de chaque mot, whisper et Azure ne les rendent pas, et l'app ne les exige nulle part — elle répartit les sons entre les mots par l'ordre et l'orthographe, à 95 %. Les bornes enrichissent ; une brique qui les exigerait s'éteindrait le jour où la reconnaissance devient locale, ce qui est sa destination.

**La reconnaissance a une destination préférée : l'appareil.** C'est le seul des trois maillons qui puisse cesser d'être distant, et y arriver retirerait un fournisseur du mur BYOK, un motif au `NonFreeNet`, et ferait que l'audio d'un tour **ne quitte plus jamais l'appareil**. Ce qui décide n'est pas seulement sa justesse sur de la parole accentuée : c'est sa **fidélité verbatim**, dont dépend l'alignement de l'apprenant sur tout ce qu'il a dit (`design/local-recognition.md`). Ce doc a écrit que `sherpa-onnx` y arriverait sans runtime nouveau, l'app embarquant déjà ONNX Runtime ; c'est faux — son paquet Android apporte le sien, et un APK n'en porte qu'un seul. **Ce qui est acté sans attendre cette mesure, c'est la forme de la couture** : l'interface de reconnaissance prend un fichier et rend des mots avec leurs bornes — ni clé dans sa signature, ni panne réseau dans son contrat, ni latence supposée. Une couture qui présume le distant se paie en réécriture le jour où le local gagne.

### Capacités déclarées, pas plus petit dénominateur commun

Il y a deux manières d'être agnostique et elles sont opposées. La première n'expose que ce que *tous* les fournisseurs savent faire : on n'exploite alors jamais ce que le meilleur a de mieux. La seconde, retenue : **chaque fournisseur déclare ce qu'il sait faire, et l'app allume ou éteint les briques en conséquence.**

Contrepartie assumée : le jeu de fonctionnalités **dépend du fournisseur choisi**. Une option indisponible doit donc **porter sa raison** dans l'interface — sinon elle passe pour un bug, et c'est l'app qu'on accusera.
...
Écarté délibérément de la première version, non par oubli : la consigne d'articulation écrite, remplacée par l'écoute d'un modèle.
