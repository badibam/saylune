...
**Correction et pertinence sortent d'un seul marquage par groupe de mots**, où chaque groupe porte une étiquette et une seule, le correct étant l'absence de marque. La correction demande *est-ce que c'est de l'anglais* — deux étiquettes, *mal formé* et *ne se dit pas*. C'est un jugement absolu, qu'aucune consigne ne déplace, et c'est ce qui le rend vérifiable au banc sur des phrases isolées. La pertinence demande *as-tu visé juste* — trois étiquettes, *à côté*, *plat*, *juste* — et c'est elle qui porte la consigne, donc tout ce qu'une activité veut exiger : parler au passé, tenir un registre, éviter un mot. C'est la seule mesure du projet qui ait un bon côté : il n'y a pas de modèle unique de la bonne phrase, donc on peut dépasser le simplement correct, là où pour le son être sur le modèle est déjà le mieux qu'on puisse faire. La correction se souligne en vaguelette, la pertinence se prend entre crochets (`design/activity-model.md`).
...
**La porte suit ce qui va être réécrit, et rien d'autre.** Elle ne se lit donc pas au marquage, qui est invariant, mais à la **note d'une aptitude** à la barre A–B, et le ciblage plus fin passe par les poids plutôt que par un réglage à elle. **Ce qui la ferme est câblé** : les aptitudes dont l'échec fait changer les mots — la correction, la pertinence, la compréhension. Celles dont l'échec laisse la phrase intacte — l'élocution, la fluidité — ne peuvent jamais la fermer : il n'y a rien à réécrire. Une activité choisit **de quelles aptitudes elle fait refaire**, jamais de quel côté ça tombe (`design/activity-model.md`). Coût assumé au réglage le plus lâche : le modèle dira d'une voix native une tournure qu'on a choisi de ne pas reprendre.

**Et la porte sur la correction ne se désarme pas**, à la différence des autres : au-delà de ne pas travailler une phrase qui va disparaître, **on ne fait pas prononcer au modèle une phrase incorrecte**. C'est ce qu'on donne à imiter, et tout le montage repose sur le fait que le modèle est la vérité.

**L'étiquette « ne se dit pas » n'est pas cette porte-là.** La porte est une décision — la phrase pourrait être synthétisée, on choisit de ne pas le faire —, alors qu'une phrase qui n'existe pas dans la langue **ne peut pas** l'être : la faire dire au modèle donnerait à imiter une non-phrase, et tout le montage repose sur le fait que le modèle est la vérité. Ce n'est donc pas une règle qu'un défi écrit mais une propriété de l'analyse, qui se lit sur un seul empan et non sur une note. Et on ne peut pas retirer l'empan pour synthétiser le reste, comme on retire une hésitation : un bafouillage est hors de la phrase, une non-phrase est dedans et demande à être remplacée.

Rien d'autre ne s'arrête dans les deux cas : l'IA répond, le fil continue, la marque s'affiche, la reformulation est proposée. Seul le canal du son reste vide, en portant sa raison, et pour cette tentative-là seulement.
...
**Les trois s'ancrent aux mêmes caractères du texte affiché** — chaque son connaît les lettres qu'il couvre, chaque syllabe sa sous-chaîne, et la mélodie se pose sur ces mêmes groupes. Ce sont trois propriétés d'une seule chaîne, pas trois analyses à superposer. C'est ce qui rend possible de les marquer **d'un seul mouvement**, et la forme retenue le fait : **la couleur ne porte jamais l'échelle**, qui est portée par la forme — les lettres teintées pour le son, un filet sous la syllabe pour l'accent, une bande au-dessus de la ligne pour la mélodie. Trois échelles lues d'un coup d'œil sans qu'aucune ne se déguise en une autre. La couleur porte le côté et la distance au neutre : l'alarme partout, et le vert de l'étiquette *juste* de la pertinence, seule mesure qui ait un bon côté. La correction se souligne alors **en vaguelette**, pour ne pas se confondre avec le filet droit de l'accent, et la pertinence se prend **entre crochets** — une enceinte, mais qui ne court pas le long du texte, puisque *juste* se marque aussi et que presque tous les groupes portent donc une étiquette. Le dessin complet est dans `design/pixel-ui.md`.
