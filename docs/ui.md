...
**Ce qui est écrit ici et ce qui est écrit dans le code.** Chaque écran porte sa forme et sa raison dans son en-tête : la charpente (`ui/Scaffold.kt`), la rangée de commandes (`Commands.kt`), les trois boutons du bas (`Buttons.kt`), le bilan du passage (`PassageNotes.kt`), le tour de l'IA et son brouillage (`AiTurn.kt`), le menu des marques (`Channels.kt`), la ligne qui nomme le tour (`TurnLabel.kt`), les cadres (`Frames.kt`), le relevé son par son (`AnalysisReadout.kt`), l'ordre de peinture du tour marqué (`MarkedTurn.kt`). Le thème porte les valeurs : la grille (`theme/Grid.kt`), les sept nombres d'un registre (`theme/Palette.kt`), les règles de calcul de couleur (`theme/Oklab.kt`), les rythmes du marquage (`theme/Rhythm.kt`), la police (`theme/Typography.kt`). La boîte de la police et sa zone privée sont dans `../font/README.md`.
