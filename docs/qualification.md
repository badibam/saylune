...
5. **Ancrage** : chaque son et chaque syllabe désignent leurs lettres dans le texte affiché, y compris les deux irrégularités (une lettre, deux sons ; une lettre, aucun son). **Se lit à `join.py -s`**, contre l'annotation à la main d'`expected.py` : la part des sons qui portent les lettres qu'un humain leur a attribuées. Les deux compteurs de `join.py -a` — sons sans aucune lettre, lettres rattachées à aucun son — restent lisibles mais ne qualifient pas : ils voient un trou, jamais un décalage, et sur le modèle sortant ils rapportent huit défauts là où l'annotation en trouve quatre-vingt-neuf. L'annotation indexe les sons **réellement décodés** sur ces rendus : elle note le modèle pour lequel elle a été écrite et aucun autre, et une grille d'une autre forme sort de la mesure au lieu d'y entrer de travers.
