...
- **De combien l'accent allonge une syllabe — non mesuré, et c'est ce qui rend le reste interprétable.** La durée d'une syllabe est juste à 14,7 % d'elle-même, 44 % au 9e décile (`docs/analysis.md`) ; ce chiffre ne se compare à rien tant qu'on ignore l'écart qu'il doit détecter. TIMIT le permet : son dictionnaire marque la syllabe accentuée, le corpus donne les bornes des mots, et celles des sons sont posées à la main. Il sert alors hors ligne, à savoir si un instrument est assez fin — rien n'en part dans l'app.
