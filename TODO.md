...
- **Affiner le modèle de sons** — le remplaçant de `timit-ipa` : CTC phonémique + label priors, dense et acoustique, seul remède aux sons que la grille perd (un sur huit, prouvé irrécupérable en lecture). Plan et escalier d'oreilles dans `docs/design/dense-grid.md`, variantes dans `docs/design/finetune-brief.md`, chiffrage rendu dans `docs/design/gpu-pricing.md` et mode opératoire dans `docs/design/gpu-runbook.md`. TIMIT est acheté (LDC, complet dans `tmp/TIMIT/`, hors git — re-téléchargeable depuis le compte LDC et à garder sur MEGA) et l'atelier est posé (`train/`, déroulé à blanc concluant : corpus entièrement décodé, un pas d'entraînement CPU fini). Reste : archiver TIMIT sur MEGA, louer (Kaggle d'abord pour V1), les vrais runs, puis qualifier chaque checkpoint par `bench/faults.py` — l'escalier s'arrête à la première marche que la mesure ne condamne pas.
