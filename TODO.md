...
- **De combien l'accent allonge une syllabe — la mesure qui décide comment la brique 7 s'écrit, à faire avant elle.** La durée d'une syllabe est juste à 14,7 % d'elle-même, 44 % au 9e décile (`docs/analysis.md`) ; ce chiffre ne se compare à rien tant qu'on ignore l'écart qu'il doit détecter. TIMIT le permet, hors ligne, rien n'en part dans l'app : son dictionnaire marque la syllabe accentuée, et les bornes des mots comme des sons sont posées à la main. Marche à suivre : sur les mêmes 200 énoncés, pour chaque mot plein de deux syllabes et plus, le rapport de durée entre la syllabe accentuée et ses voisines, pris sur les bornes vraies — c'est l'écart à détecter, à mettre en face de l'erreur de l'instrument (29,1 ms en médiane, 44 % au 9e décile). Les deux issues s'écrivent d'avance pour ne pas se négocier devant le chiffre : si l'allongement dépasse nettement l'erreur au 9e décile, la durée sert comme conçue ; sinon la brique 7 se réécrit autour de la réduction vocalique comme trait premier — déjà classée la plus forte en anglais, et gratuite puisque c'est la grille qui l'écrit — la durée ne restant qu'en corroboration.
- **La syllabe que l'apprenant ajoute — sa piste est distincte, et elle est mesurée.** Décoder librement l'apprenant et lui passer la jointure comme au modèle : la matrice est déjà calculée pour l'alignement forcé, le décodage libre n'en est qu'un argmax, et la jointure ne consulte pas d'horloge. C'est une lecture de plus, pas un calcul de plus, et elle situe l'écart **sur un mot**.
