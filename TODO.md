...
- **Les vingt-sept prises étiquetées n'ont aucune sauvegarde — à régler, décision utilisateur.** `bench/out/` entier est gitignoré sous le commentaire « regenerable by script », et `bench/out/takes` (5,2 Mo, enregistrées au micro, non refaisables) est dedans : la décision « on ne sauvegarde pas » couvrait les rendus, pas elles. La norme `universel` demande une stratégie explicite pour le durable non régénérable hors git. Recommandation : les versionner (5,2 Mo, git les porte sans peine) en les sortant de `bench/out/` ou en trouant le `.gitignore` ; à valider avant de toucher.
