...
- **Affiner le modèle de sons** — le remplaçant de `timit-ipa` : CTC phonémique + label priors, dense et acoustique, seul remède aux sons que la grille perd (un sur huit, prouvé irrécupérable en lecture). Plan et escalier d'oreilles dans `docs/design/dense-grid.md`, variantes et chiffrage GPU dans `docs/design/finetune-brief.md`. Préalables matériels : achat TIMIT (~250 $, LDC), location GPU.
