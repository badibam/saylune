...
- **Détecter la syllabe en trop ou en moins.** La grille compte les syllabes du **modèle** ; l'apprenant est plaqué dessus, donc une syllabe qu'il ajoute n'a pas de case et ne se voit pas. Mesuré sur la prise 10 (`comfortable` en quatre syllabes) : l'écart monte à 0,899 sur le `ɝ` du mot, mais la prise témoin de la même phrase par la même personne est déjà à 0,718 — aucune séparation. Il faut donc **compter**, pas comparer des formes : le compte des lettres dit combien de syllabes le mot peut porter, le décodage libre de l'apprenant (brique 11) dit combien il en a dites. C'est une brique de plus, à côté de la 7, et c'est la seule route vers cette faute — qui est de la non-réduction, la famille que le tour entier a déjà montrée.
...
- **Construire l'instrument qui manque : `bench/divergence.py`.** Deux modèles ou plus sur les mêmes audios, on aligne leurs suites de sons, on garde les endroits de désaccord, on joue le **mot entier** — soixante millisecondes ne se jugent pas isolées, c'est la leçon de `review.py` — et l'auteur dit qui a raison. Sur les voix de synthèse **et** sur les prises humaines : la première est ce que l'app consomme tous les jours, la seconde vérifie que la fidélité tient sur une vraie bouche. Les sons **manquants**, eux, se comptent sans oreille (`syllables.py`, `missing_mass.py`).
...
- `bench/out/renders/` est traité comme un cache régénérable, et il ne l'est pas. **À sauvegarder hors du dépôt comme les prises**, sans quoi aucun chiffre du banc n'est comparable d'un mois à l'autre.
