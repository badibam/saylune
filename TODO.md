...
- **Pour la brique 7, lire le rythme de l'alignement existant** : la déformation locale de la correspondance temporelle modèle ↔ apprenant (l'un traîne sur une syllabe, avale la suivante), une fois la pente d'ensemble — le débit — retirée, est un signal de durée pour l'accent lexical — à peser contre ce que la durée vaut, mesuré plus bas. Les positions des sons des deux côtés sortent déjà de la matrice : c'est une lecture de plus, pas un calcul de plus.
...
- **La centralisation lue comme une quantité ne marche pas.** Lire la masse de schwa sous chaque noyau au lieu de l'étiquette binaire : **63,1 %**, et 58,1 % sur les cas à départager — sous la durée. Sans étiquette du tout, la forme seule fait 58,2 %. La réduction se comporte comme un interrupteur, pas comme un curseur. (Erreur de raisonnement à ne pas refaire : la règle « comparer des formes, pas des maximums » a été écrite pour comparer **l'apprenant au modèle sur la même chose** ; ici on comparait deux sons différents d'un même côté, hors de sa portée.)
...
Deux causes se cumulent, et aucune n'est réparée :

- **Le montage aligné prive la réduction de tout pouvoir.** L'apprenant est forcé sur la suite de sons du modèle, donc il a des plages mais pas de symboles : le test de réduction lit les mêmes étiquettes des deux côtés et rend forcément le même verdict. Le repère à 97,5 % ne peut rien détecter, et il ne reste que la durée et l'intensité.
- **Le montage à deux décodages libère la réduction mais fait tourner la jointure sur la voix d'apprenant**, où elle n'a jamais été mesurée — les 95 % de la brique 4 sont lus sur des rendus de synthèse. Ça se voit à l'œil : `like` reçoit `ɝ n l aɪ k h`, dont le début appartient au mot d'avant et la fin au suivant. Conséquence chiffrée : sur 306 mots, **157 n'ont pas le même compte de syllabes des deux côtés**, et 154 d'entre eux ont l'accent noté correct — cette branche marquerait à 98 % à tort.

### La brique 7 — les pistes, présentées comme telles

Rien de ce qui suit n'est mesuré ; ce sont des directions, avec ce qui les bloque.

- **Écrire la règle des voyelles rhotiques dans le compteur de noyaux.** `ɪ`+`ɝ` (`here`), `ɛ`+`ɝ` (`chair`) sont **un seul** noyau et en valent deux aujourd'hui. Petit, déjà repéré sous le cas `chair`, et il touche aussi la brique 8 qui utilise le même test. À faire en premier : une part des 157 mots au compte discordant vient de là.
- **Mesurer la jointure sur la voix d'apprenant.** C'est l'inconnue qui décide si le montage à deux décodages tient debout, donc si la brique 7 récupère son meilleur repère. Le corpus L2 le permet et l'annotation nécessaire est le texte lui-même.
- **Comparer les profils plutôt que désigner une syllabe.** Ne nommer aucune gagnante : chaque côté rend deux formes sur les syllabes du mot — durées relatives, intensités relatives des noyaux — et on compare forme à forme par la divergence de la brique 3. Essayé (`bench/accent.py`), non concluant : sur les trois seules fautes d'accent comparables, les divergences sortent à 0,000–0,017 quand les mots corrects vont jusqu'à 0,285. Trois cas ne décident rien, mais rien ne sépare pour l'instant. À reprendre quand les deux points ci-dessus seront réglés et sur assez de fautes — la moitié `test` en porte 127, contre 3 dans le tirage lu.
- **La dynamique de la voix de synthèse.** L'intensité étant devenue le trait qui tranche, il faut savoir si une voix de synthèse la porte comme une bouche. Première lecture sur **9 mots seulement** : le contraste d'intensité dans le mot est de 2,03× côté modèle contre 1,14× côté prise — c'est la synthèse qui est la plus contrastée, l'inverse de ce qu'on craignait. Deux explications non départagées : soit la synthèse marque vraiment plus, soit les prises du banc sont plates parce qu'elles sont lues avec application par une seule personne. Se tranche sur le corpus L2, 125 locuteurs.
- **La hauteur**, gardée pour la fin : `docs/analysis.md` a déjà l'argument qui la relègue — une syllabe accentuée ne reçoit un pic que si le mot est en relief dans la phrase.
