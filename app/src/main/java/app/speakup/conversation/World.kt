...
import app.speakup.notes.Letter
...
import app.speakup.rules.Facts
...
class World(
    ...
    private val clocks: Map<app.speakup.rules.Trigger.Clock.Which, Int> = emptyMap(),
    private val draw: Random = Random.Default,
) : Facts {

    override fun clock(which: app.speakup.rules.Trigger.Clock.Which): Int = clocks[which] ?: 0
     ...
     */
    override fun node(of: app.speakup.rules.Trigger.Node): Boolean =
        when (of.reads) {
            app.speakup.rules.Trigger.Node.Reads.Note -> note(of.path)?.let {
            ...
            app.speakup.rules.Trigger.Node.Reads.Figure -> figure(of.path)?.let {
                it >= of.value.toFloat()
            } ?: false
            ...
            // the figure.
            app.speakup.rules.Trigger.Node.Reads.Element -> material.any(of.path, of.value)
        }
    ...
    override fun chose(rule: Rule): Int = when (rule.decider) {
        app.speakup.rules.Decider.Chance -> draw.nextInt(rule.choice.size)
        else -> 0
    }

    /** The note of a node, over the sheets under it. Null where nothing under it measured. */
    private fun note(path: String): app.speakup.notes.Note? {
