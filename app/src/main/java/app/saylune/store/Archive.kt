            ...
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL(
                    "INSERT INTO `activities_new` SELECT `id`, `matter`, '{}', NULL, NULL, " +
                        "'[]', '[]', '[]', NULL, ${app.saylune.activity.Activity.ENGINE}, " +
            ...
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "UPDATE `utterances` SET `speaker` = " +
                        "'${app.saylune.conversation.Speaker.LEARNER}' WHERE `speaker` = 'Learner'",
                )
                db.execSQL(
                    "UPDATE `utterances` SET `speaker` = " +
                        "'${app.saylune.conversation.Speaker.SAYLUNE}' WHERE `speaker` = 'Ai'",
                )
            }
        }
