...
import app.saylune.ui.theme.Typography
...
private fun Play(label: String, modifier: Modifier, onClick: () -> Unit) {
    ...
    Row(
        ...
        Text(label, style = Saylune.type.text, color = palette.ink.srgb, maxLines = 1)
        Text(Glyphs.PLAY.toString(), style = Saylune.type.text, color = palette.ink.srgb)
    }
}
...
private fun Gauge(part: Float, colour: Color, columns: Int) {
    ...
    Box(Modifier.width(grid.cell * columns)) {
        Text(
            ...
            style = Saylune.type.text, color = colors.dim, maxLines = 1,
        )
        Text(
            ...
            style = Saylune.type.text, color = colour, maxLines = 1,
        )
    }
}
