...
class UnfoldTest {
    ...
    private val correctness = Sheets.columnOf("correction/correction")
    private val relevance = Sheets.columnOf("pertinence/pertinence")
    private val stumbling = Sheets.columnOf("fluidite/remplissage-reprises")
    ...
    @Test fun `a span unfolds into the words it covers`() {
        ...
        val words = unfoldWith(text, listOf(Marked(0, 4, "mal-forme")), correctness)
        assertEquals(listOf("mal-forme", "mal-forme", "ok", "ok"), words.map { it.notch })
    ...
    @Test fun `two overlapping spans give a word one notch, the worst it carries`() {
        ...
        val words = unfoldWith(
            text,
            listOf(Marked(0, 10, "mal-forme"), Marked(2, 4, "ne-se-dit-pas")),
            correctness,
        )
        // `ne se dit pas` before `mal formé` before `ok`, and `go` counts once.
        assertEquals(listOf("mal-forme", "ne-se-dit-pas", "mal-forme", "ok"), words.map { it.notch })
    }
    ...
    @Test fun `on relevance the precedence runs from a-cote down to juste`() {
        ...
        val words = unfoldWith(
            text,
            listOf(Marked(0, 8, "juste"), Marked(0, 16, "a-cote")),
            relevance,
        )
        ...
        assertTrue(words.all { it.notch == "a-cote" })
    }

    @Test fun `an unmarked spoken word is kept`() {
        val text = "It was like um I went"
        val words = unfoldWith(text, listOf(Marked(7, 14, "remplissage")), stumbling)
        assertEquals(
            listOf("retenu", "retenu", "remplissage", "remplissage", "retenu", "retenu"),
            words.map { it.notch },
        )
    }
    ...
    @Test fun `bounds that miss a word boundary fail outright`() {
        ...
        assertTrue(runCatching {
            unfoldWith(text, listOf(Marked(1, 4, "mal-forme")), correctness)
        }.isFailure)
        assertTrue(runCatching {
            unfoldWith(text, listOf(Marked(0, 3, "mal-forme")), correctness)
        }.isFailure)
    }
    ...
    @Test fun `punctuation travels with the word it touches`() {
        ...
        val marked = unfoldWith(text, listOf(Marked(0, 5, "plat")), relevance)
        assertEquals("plat", marked.first().notch)
    }
    ...
    @Test fun `the figure is the mean of the words the sheet reads`() {
        ...
        val words = unfoldWith(text, listOf(Marked(39, 43, "plat")), relevance)
