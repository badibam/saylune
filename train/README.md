...
`train/fetch.py` rapatrie les checkpoints d'un notebook Kaggle, un fichier à la fois et de façon reprenable — `train/fetch.py --kernel <compte>/<notebook> --epoch 029`, la même commande relancée reprenant un transfert coupé. `--kernel` n'a pas de défaut : un notebook mémorisé désignerait toujours le run précédent. **C'est l'utilisateur qui le lance**, pas la session d'IA ; le pourquoi et les voies mortes sont dans `../docs/design/gpu-runbook.md`.
